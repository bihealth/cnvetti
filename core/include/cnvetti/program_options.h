...
public:
    ...
    // Step size of GC windows.
    double gcStepSize;

    // Minimal alignment quality to consider.
    int minAlignmentQuality;

    // Whether or not to compute bin stdev values
    bool computeBinStdevs;

    // Whether to ignore read pairs but those aligning on the same contig in
    // inward orientation.
    bool ignoreDiscordantPairs;

    // Path to sample-specific, tabix-indexed, bgzip-ed peaks BED file
    // generated by "cnvetti peaks", if any.
    std::string peaksBedFile;

    // Path to target region file. Will only count in bins that overlap with
    // BED file, if given.
